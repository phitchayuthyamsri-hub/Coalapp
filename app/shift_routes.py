...
def _dmy(iso):
    """YYYY-MM-DD, the stored form, as DD/MM/YYYY, the form people read.

    For words the server writes itself - messages and exports. Data the pages
    receive stays YYYY-MM-DD, because it is sorted and compared as text."""
    s = str(iso or "")
...
def upload():
    ...
    if dl and dl.state == "confirmed":
        ...
        return jsonify(error="The list for %s on %s is already confirmed. It "
                             ...
                             % ((s.name if s else "this company"), _dmy(day))), 409
...
def week_export():
    ...
    for s in d["summary"]:
        ...
        ws2.append([day, day.strftime("%a"), s["trucks"], s["entering"],
    ...
    ws3.append(["Generated", (datetime.utcnow() + LOCAL_OFFSET).strftime("%d/%m/%Y %H:%M")])
...
def amend_request():
    ...
    dl = _find_list(day, sub_id)
    if not dl:
        return jsonify(error="There is no list for %s to amend." % _dmy(day)), 404
    ...
    for sub in Subcontractor.query.filter_by(active=True).order_by(Subcontractor.name).all():
        ...
        for k in sorted(set(fleet) - set(planned), key=lambda x: fleet[x]):
            why = (_why_not_assigned(rows.get(k), day) if planning is not None
                   else "No sheet declared for %s" % _dmy(day))
