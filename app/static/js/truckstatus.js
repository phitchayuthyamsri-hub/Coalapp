...
async function load(){
  ...
  hr.innerHTML=noHead()+'<th class="rowhead">Plate</th>'+d.dates.map(dt=>`<th>${dt.slice(5)}</th>`).join('');
  th.appendChild(hr);
  d.plates.forEach((p,i)=>{const tr=document.createElement('tr');
    tr.innerHTML=`${rowNo(i)}<td class="rowhead">${p}</td>`+d.dates.map(dt=>{
